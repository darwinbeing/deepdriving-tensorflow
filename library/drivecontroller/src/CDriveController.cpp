...
void CDriveController::controlLane2(Indicators_t const &rIndicators, Control_t &rControl)
{
  ...
  if (lane_change==0 && rIndicators.DistMM<15) {   // if current lane is occupied
    ...
    if (rIndicators.LL>-8 && left_clear==1 && steer_trend>=0) {   // move to left lane
      lane_change=-2;
      coe_steer=6;
      right_clear=0;
      right_timer=0;
    ...
    else if (rIndicators.RR<8 && right_clear==1 && steer_trend<=0) {   // move to right lane
      lane_change=2;
      coe_steer=6;
      left_clear=0;
      left_timer=0;
    ...
    if (rIndicators.LL<-8 && right_clear==1 && steer_trend<=0 && steer_trend>-0.2) {  // in left lane, move to right lane
      lane_change=2;
      coe_steer=6;
      right_clear=0;
      right_timer=20;
